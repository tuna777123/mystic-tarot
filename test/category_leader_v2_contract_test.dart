...
void main() {
  test('category leader v2 benchmark locks the evidence-first moat', () {
    final benchmark = File('docs/CATEGORY_LEADER_V2_2026_08_31.md').readAsStringSync();

    expect(benchmark, contains('Reality Evidence, not prediction accuracy'));
    expect(benchmark, contains('Require at least three completed Mirror check-ins'));
  ...
  test('production materializer replaces prediction-like Journal insights', () {
    final materializer = File('tool/materialize_ad_only_ui.dart').readAsStringSync();
    ...
    expect(materializer, contains("journalSource.contains('Movement noticed')"));
