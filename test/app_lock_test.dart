...
void main() {
  ...
  test(
    'wrong PIN is rejected and persistent delay begins at attempt five',
    () async {
      final store = MemoryAppLockStore();
      final service = testAppLockService(store);
      ...
      await expectLater(
        service.verifyPin('135790', now: now.add(const Duration(seconds: 1))),
  ...
  test(
    'trusted unlock clears failed attempts and disable removes lock data',
    () async {
      final store = MemoryAppLockStore();
      final service = testAppLockService(store);
      ...
      store.values['mystic.app-lock.locked-until.v1'] = DateTime.utc(
        2026,
        8,
        4,
        13,
