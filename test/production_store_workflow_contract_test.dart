...
void main() {
  test(
    'production store workflow secret refs match release contract exactly',
    () {
      final workflow = File(
  ...
  test(
    'signed store jobs remain behind the protected production environment',
    () {
      final workflow = File(
        '.github/workflows/store-release.yml',
      ).readAsStringSync();
  ...
  test(
    'production workflow keeps fail-closed preflight and locked dependencies',
    () {
      final workflow = File(
        '.github/workflows/store-release.yml',
      ).readAsStringSync();
      ...
      expect(
        workflow,
        contains(
          'dart run tool/store_release_preflight.dart --platform=android',
        ),
      );
      expect(
        workflow,
      ...
      expect(
        RegExp(
          r'run:\s*flutter pub get --enforce-lockfile',
        ).allMatches(workflow).length,
        3,
        reason:
      ...
      expect(
        RegExp(
          r'--dart-define=MYSTIC_USE_TEST_ADS=false',
        ).allMatches(workflow).length,
        2,
        reason:
