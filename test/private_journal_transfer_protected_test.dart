...
void main() {
  ...
  test('service creates protected V2 codes and requires the passphrase',
      () async {
    final service = PrivateJournalTransferService(
      preferences: await SharedPreferences.getInstance(),
    );
    final code = await service.createCode(
    ...
    expect(preview.mergedRecords.single.question, 'What deserves protection?');
  });
  ...
  test('legacy V1 codes remain restorable without a passphrase', () async {
    ...
    final code = JournalTransferCodec.encode(records: <ReadingRecord>[_record()]);
