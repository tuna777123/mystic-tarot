...
void main() {
  test('interstitial opportunity persists until a real impression', () {
    ...
    final recordStart = source.indexOf(
      'Future<void> _recordCompletedReading()',
    );
    ...
    expect(source, contains('ad_completed_readings_since_interstitial_v1'));
    expect(recordSection, contains('_completedReadingsSinceInterstitialKey'));
    expect(
      recordSection,
      isNot(contains('_completedReadingsSinceAd = 0;')),
      reason: 'A load miss or cooldown must not consume a due impression.',
    );
    expect(showSection, contains('onAdImpression: (_) {'));
    expect(
      showSection,
