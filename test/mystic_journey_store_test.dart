...
void main() {
  ...
  test('saves and loads journeys', () async {
    ...
    final store = SharedPreferencesMysticJourneyStore(preferences: preferences);
  ...
  test(
    'preserves the previous snapshot before replacing primary data',
    () async {
      final preferences = await SharedPreferences.getInstance();
  ...
  test(
    'returns a safe empty result when both snapshots are corrupted',
    () async {
      SharedPreferences.setMockInitialValues({
  ...
  test('clears primary and backup snapshots', () async {
    ...
    expect(
      preferences.containsKey(SharedPreferencesMysticJourneyStore.primaryKey),
      isFalse,
    );
    expect(
      preferences.containsKey(SharedPreferencesMysticJourneyStore.backupKey),
      isFalse,
    );
  });
}
