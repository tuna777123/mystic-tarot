...
void main() {
  test('iOS CI audits and uploads privacy manifest evidence', () {
    ...
    expect(
      workflow,
      contains('--app=build/ios/iphoneos/Runner.app'),
    );
    expect(
      workflow,
    ...
    expect(
      workflow,
      contains('mystic-tarot-ios-privacy-manifest-evidence'),
    );
  });

  test('signed iOS release manifest fails closed through IPA privacy audit', () {
    final writer = File('tool/write_release_manifest.dart').readAsStringSync();
    ...
    expect(
      writer,
      contains("import 'src/ios_privacy_manifest_audit.dart';"),
    );
    ...
    expect(
      writer,
      contains('ios-privacy-manifest-evidence.json'),
    );
    expect(
      productionWorkflow,
