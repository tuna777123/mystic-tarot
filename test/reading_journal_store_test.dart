...
Map<String, dynamic> damagedPayload({
  ...
  final payload =
      jsonDecode(ReadingJournalCodec.encode(<ReadingRecord>[validRecord]))
...
void main() {
  ...
  test('codec rejects one damaged record without losing valid history', () {
    final report = ReadingJournalCodec.decode(
      jsonEncode(
        damagedPayload(validRecord: recordAt(DateTime.utc(2026, 8, 2, 12))),
      ),
    );
  ...
  test(
    'store preserves the previous snapshot before replacing primary data',
    () async {
  ...
  test(
    'store recovers from backup when primary payload is unreadable',
    () async {
  ...
  test(
    'store recovers from backup when every primary record is damaged',
    () async {
  ...
  test(
    'partial primary corruption is repaired with missing backup history',
    () async {
      ...
      expect(result.recoveredFromBackup, isTrue);
      expect(result.records.map((record) => record.question), <String>[
        'Newest primary',
        'Older backup',
      ]);
      expect(result.rejectedItems, 1);
  ...
  test(
    'primary records win when the backup contains the same identity',
    () async {
  ...
  test(
    'legacy string-list records migrate while corrupt items are skipped',
    () async {
      final legacyRecord = jsonEncode(<String, Object>{
        'kind': ReadingKind.daily.name,
        'question': 'Legacy question',
        'cards': <Map<String, Object>>[
          <String, Object>{'name': tarotDeck.first.name, 'reversed': false},
        ],
  ...
  test('duplicate record identities are rejected deterministically', () {
    ...
    final payload =
        jsonDecode(ReadingJournalCodec.encode(<ReadingRecord>[record]))
            as Map<String, dynamic>;
    final records = payload['records'] as List<dynamic>;
