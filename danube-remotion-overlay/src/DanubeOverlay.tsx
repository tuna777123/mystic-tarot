...
const beats:Beat[]=[
 ...
 {s:20.04,e:22.10,top:'SO THEY DID',bottom:'SOMETHING EXTREME',accent:'gold',pos:'low'},
 {s:22.10,e:24.64,top:'THEY SANK',bottom:'THEIR OWN SHIPS',accent:'gold',pos:'low'},
 {s:26.40,e:28.16,top:'SCUTTLED ACROSS',bottom:'THE DANUBE',accent:'gold',pos:'high',align:'left'},
 ...
 {s:35.05,e:38.24,top:'BENEATH THE WATER',bottom:'UNTIL NOW',accent:'gold',pos:'low'},
 {s:38.24,e:40.40,top:'EXTREME',bottom:'DROUGHT',accent:'gold',pos:'low'},
 {s:40.40,e:42.45,top:'RECORD-LOW',bottom:'WATER LEVELS',accent:'gold',pos:'low'},
 {s:44.90,e:47.82,top:'SOME ALMOST',bottom:'OUT OF THE WATER',accent:'gold',pos:'low'},
...
const stats:Stat[]=[
 ...
 {s:24.64,e:26.40,big:'≈200',label:'VESSELS',sub:'SCUTTLED'},
 {s:32.70,e:35.05,big:'80+',label:'YEARS HIDDEN',sub:'BENEATH THE WATER',side:'right'},
 {s:42.45,e:44.90,big:'20+',label:'WRECKS EXPOSED',sub:'AT RECORD-LOW WATER'},
];
...
const Caption:React.FC<{b:Beat}>=({b})=>{
 ...
 const yIn=interpolate(local,[0,6],[16,0],{...clamp,easing:ease});
 const yOut=interpolate(local,[Math.max(0,d-4),d],[0,-8],{...clamp,easing:ease});
 const align=b.align??'center'; const pos=b.pos??'low';
 const top=pos==='high'?260:pos==='mid'?920:1250;
 ...
 return <div style={{position:'absolute',left:align==='left'?64:70,right:70,top,width:align==='left'?860:940,textAlign:align,transform:`translate3d(0,${yIn+yOut}px,0)`}}>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:68,lineHeight:.92,letterSpacing:-1.5,color:WHITE,textTransform:'uppercase'}}>{b.top}</div>
   {b.bottom&&<div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:76,lineHeight:.91,letterSpacing:-2,color,marginTop:5,textTransform:'uppercase'}}>{b.bottom}</div>}
 </div>;
};
...
const BigStat:React.FC<{c:Stat}>=({c})=>{
 ...
 const xIn=interpolate(local,[0,7],[left?-34:34,0],{...clamp,easing:ease});
 const xOut=interpolate(local,[Math.max(0,d-4),d],[0,left?-10:10],{...clamp,easing:ease});
 return <div style={{position:'absolute',top:190,left:left?58:undefined,right:left?undefined:58,width:760,textAlign:left?'left':'right',transform:`translate3d(${xIn+xOut}px,0,0)`}}>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:182,lineHeight:.8,letterSpacing:-7,color:WHITE}}>{c.big}</div>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:42,lineHeight:.95,letterSpacing:2.2,color:GOLD,marginTop:18,textTransform:'uppercase'}}>{c.label}</div>
   {c.sub&&<div style={{...outline,fontFamily:FONT,fontWeight:800,fontSize:25,lineHeight:1.05,letterSpacing:1.7,color:WHITE,marginTop:10,textTransform:'uppercase'}}>{c.sub}</div>}
 </div>;
};
...
const Danger:React.FC=()=>{
 ...
 const local=f-s,d=e-s;
 const yIn=interpolate(local,[0,7],[18,0],{...clamp,easing:ease});
 const yOut=interpolate(local,[Math.max(0,d-4),d],[0,-8],{...clamp,easing:ease});
 return <div style={{position:'absolute',left:64,right:64,top:1115,transform:`translate3d(0,${yIn+yOut}px,0)`}}>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:100,lineHeight:.84,letterSpacing:-3,color:RED}}>LIVE</div>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:92,lineHeight:.86,letterSpacing:-2.6,color:WHITE}}>AMMUNITION</div>
   <div style={{...outline,fontFamily:FONT,fontWeight:900,fontSize:32,lineHeight:1,letterSpacing:2.6,color:RED,marginTop:13}}>AND EXPLOSIVES</div>
 </div>;
};
