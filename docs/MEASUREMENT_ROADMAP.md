...
The local evidence model stores only coarse event/day counts and approved business dimensions. Private tarot content is never an analytics dimension.

## What is measurable locally

- app opens / active calendar days;
- onboarding completion;
- genuinely new saved readings;
- Mirror due surfacing;
- raw Mirror completion timing;
- **mature 72-hour Mirror windows**;
...
Each reading becomes eligible for the mature cohort denominator only after its full 72-hour completion window closes. At that moment the local tracker emits one deduped `mirrorWindowMatured` aggregate event, classified as:
...
D1, D7 and D30 are evaluated only when an evidence file is old enough for the corresponding denominator. A missing or immature denominator is never interpreted as a pass.
...
The aggregator rejects unexpected privacy models, internal dedupe fields and impossible Mirror numerator/denominator states.
...
3. Keep the source-level private-field rejection boundary.
