...
class _MysticPaywallScreenState extends State<MysticPaywallScreen> {
        ...
        child: ListView(
          ...
          children: [
            const Text(
              ...
              style: TextStyle(fontSize: 54, color: MysticColors.gold),
            ),
            const SizedBox(height: 10),
            Text(
              'Turn every reading into a path that remembers you.',
              textAlign: TextAlign.center,
              style: Theme.of(context).textTheme.headlineMedium,
            ),
            const SizedBox(height: 10),
            Text(
              _sourceMessage(widget.source),
              textAlign: TextAlign.center,
            ...
            const _BenefitRow(
              ...
              title: 'Unlimited deep readings',
              subtitle: 'Love, career, money, shadow work, and more.',
            ),
            const _BenefitRow(
              icon: Icons.auto_awesome,
              title: 'Unlimited Oracle dialogue',
              subtitle: 'Keep exploring after the first interpretation.',
            ),
            const _BenefitRow(
              icon: Icons.hub_outlined,
              title: 'Full Living Fate insights',
              subtitle:
                  'See recurring cards, emotional shifts, and transitions.',
            ),
            const _BenefitRow(
              icon: Icons.lock_outline,
              title: 'Private by design',
              subtitle: 'Your journal remains on your device.',
            ),
            const SizedBox(height: 18),
            ...MysticOffer.launchCatalog.map(
              (offer) => Padding(
            ...
            TextButton(
              onPressed:
                  widget.onRestore == null || status == PurchaseStatus.loading
                  ? null
                  : _restore,
  ...
  Future<void> _purchase() async {
    ...
    if (result == PurchaseStatus.purchased ||
        result == PurchaseStatus.restored) {
  ...
  Future<void> _restore() async {
    ...
    if (!mounted) return;
    setState(() => status = result);
    if (result == PurchaseStatus.restored ||
  ...
  String? _statusMessage() => switch (status) {
    ...
    PurchaseStatus.failed =>
      'The store could not complete the purchase. Try again later.',
    PurchaseStatus.pending => 'Your store is still processing this purchase.',
    PurchaseStatus.refunded =>
      'This subscription was refunded and Plus access is inactive.',
    ...
    child: Row(
      children: [
        Container(
          ...
          child: Icon(icon, color: MysticColors.gold, size: 21),
        ),
      ...
      decoration: BoxDecoration(
        gradient: selected
            ? const LinearGradient(
                colors: [Color(0xFF654491), Color(0xFF291A3D)],
              )
            : null,
        color: selected ? null : Colors.white.withValues(alpha: .045),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
          color: selected ? MysticColors.gold : Colors.white12,
...
String _sourceMessage(PaywallSource source) => switch (source) {
  PaywallSource.dailyLimit =>
    'You reached today’s free deep-reading limit. Keep your path moving with unlimited access.',
  PaywallSource.premiumSpread =>
    'Unlock the complete spreads built for the questions that need more depth.',
  PaywallSource.oracleDialogue =>
    'Continue the Oracle conversation without stopping after the first answer.',
  PaywallSource.profile =>
    'Upgrade your personal practice with every Mystic Plus feature.',
  _ =>
    'Unlock the complete Mystic experience and let your path grow over time.',
};
