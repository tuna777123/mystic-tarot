...
class LivingFateSnapshot {
  ...
  CardFrequency? get mostRecurringCard =>
      frequencies.isEmpty ? null : frequencies.first;
...
class LivingFateEngine {
  ...
  LivingFateSnapshot analyze(List<ReadingRecord> records, {DateTime? now}) {
    ...
    final transitions =
        transitionCounts.entries.map((entry) {
    ...
    final dueCheckIns =
        chronological
            .map(
              (record) =>
                  FateCheckIn(record: record, dueAt: record.mirrorCheckInAt),
            )
    ...
    if (kindCounts.isNotEmpty) {
      dominantKind = kindCounts.entries
          .reduce((a, b) => a.value >= b.value ? a : b)
          .key;
    }
