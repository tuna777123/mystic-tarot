...
String _contextualMeaning({
  ...
  final frame = switch (kind) {
    ReadingKind.daily => _copy(
      language,
      en: 'For today, in ${position.toLowerCase()}, notice how $theme is showing up before you react automatically.',
      tr: 'Bugün ${position.toLowerCase()} konumunda $theme temasının otomatik tepki vermeden önce nasıl ortaya çıktığını fark et.',
      es: 'Para hoy, en ${position.toLowerCase()}, observa cómo aparece $theme antes de reaccionar en automático.',
      fr: 'Pour aujourd’hui, dans ${position.toLowerCase()}, observez comment $theme se manifeste avant de réagir automatiquement.',
      pt: 'Para hoje, em ${position.toLowerCase()}, observe como $theme aparece antes de reagir no automático.',
    ),
    ReadingKind.love => _copy(
      language,
      en: 'In a relationship context, within ${position.toLowerCase()}, test how $theme affects reciprocity, boundaries, and what is actually being communicated.',
      tr: 'İlişki bağlamında ${position.toLowerCase()} konumunda $theme temasının karşılıklılığı, sınırları ve gerçekten neyin iletişim kurulduğunu nasıl etkilediğini test et.',
      es: 'En un contexto relacional, dentro de ${position.toLowerCase()}, prueba cómo $theme afecta la reciprocidad, los límites y lo que realmente se está comunicando.',
      fr: 'Dans un contexte relationnel, dans ${position.toLowerCase()}, observez comment $theme agit sur la réciprocité, les limites et ce qui est réellement communiqué.',
      pt: 'Em um contexto de relacionamento, em ${position.toLowerCase()}, teste como $theme afeta reciprocidade, limites e o que está sendo realmente comunicado.',
    ),
    ReadingKind.career => _copy(
      language,
      en: 'In a career context, within ${position.toLowerCase()}, test how $theme affects responsibility, leverage, skill, and the next move you can control.',
      tr: 'Kariyer bağlamında ${position.toLowerCase()} konumunda $theme temasının sorumluluğu, hareket alanını, beceriyi ve kontrol edebileceğin sıradaki hamleyi nasıl etkilediğini test et.',
      es: 'En un contexto profesional, dentro de ${position.toLowerCase()}, prueba cómo $theme afecta la responsabilidad, tu margen de acción, la habilidad y el siguiente movimiento que puedes controlar.',
      fr: 'Dans un contexte professionnel, dans ${position.toLowerCase()}, observez comment $theme agit sur la responsabilité, votre marge de manœuvre, la compétence et la prochaine action contrôlable.',
      pt: 'Em um contexto profissional, em ${position.toLowerCase()}, teste como $theme afeta responsabilidade, margem de ação, habilidade e o próximo movimento que você controla.',
    ),
    ReadingKind.money => _copy(
      language,
      en: 'In a money context, within ${position.toLowerCase()}, test how $theme affects cash, risk, time, and the resources you can sustain.',
      tr: 'Para bağlamında ${position.toLowerCase()} konumunda $theme temasının nakdi, riski, zamanı ve sürdürebildiğin kaynakları nasıl etkilediğini test et.',
      es: 'En un contexto financiero, dentro de ${position.toLowerCase()}, prueba cómo $theme afecta el dinero disponible, el riesgo, el tiempo y los recursos que puedes sostener.',
      fr: 'Dans un contexte financier, dans ${position.toLowerCase()}, observez comment $theme agit sur la trésorerie, le risque, le temps et les ressources que vous pouvez soutenir.',
      pt: 'Em um contexto financeiro, em ${position.toLowerCase()}, teste como $theme afeta caixa, risco, tempo e os recursos que você consegue sustentar.',
    ),
    ReadingKind.decision => _copy(
      language,
      en: 'For this decision, in ${position.toLowerCase()}, use $theme to compare trade-offs instead of searching for a yes-or-no command.',
      tr: 'Bu kararda ${position.toLowerCase()} konumunda evet-hayır emri aramak yerine $theme temasını seçeneklerin bedellerini karşılaştırmak için kullan.',
      es: 'Para esta decisión, en ${position.toLowerCase()}, usa $theme para comparar costes y beneficios en vez de buscar una orden de sí o no.',
      fr: 'Pour cette décision, dans ${position.toLowerCase()}, utilisez $theme pour comparer les compromis au lieu de chercher une réponse oui ou non.',
      pt: 'Para esta decisão, em ${position.toLowerCase()}, use $theme para comparar trocas e custos em vez de procurar uma ordem de sim ou não.',
    ),
    ReadingKind.spiritual => _copy(
      language,
      en: 'For spiritual growth, in ${position.toLowerCase()}, notice how $theme appears in practice rather than treating symbolism as proof.',
      tr: 'Ruhsal gelişimde ${position.toLowerCase()} konumunda sembolizmi kanıt saymak yerine $theme temasının günlük pratikte nasıl göründüğünü fark et.',
      es: 'Para el crecimiento espiritual, en ${position.toLowerCase()}, observa cómo $theme aparece en la práctica en lugar de tratar el simbolismo como prueba.',
      fr: 'Pour la croissance spirituelle, dans ${position.toLowerCase()}, observez comment $theme apparaît dans la pratique au lieu de traiter le symbole comme une preuve.',
      pt: 'Para o crescimento espiritual, em ${position.toLowerCase()}, observe como $theme aparece na prática em vez de tratar simbolismo como prova.',
    ),
    ReadingKind.shadow => _copy(
      language,
      en: 'For shadow work, in ${position.toLowerCase()}, look for observable behavior around $theme, especially what you avoid, over-control, or repeat.',
      tr: 'Gölge çalışmasında ${position.toLowerCase()} konumunda $theme çevresindeki gözlemlenebilir davranışlara, özellikle kaçındığın, aşırı kontrol ettiğin veya tekrarladığın şeylere bak.',
      es: 'Para el trabajo de sombra, en ${position.toLowerCase()}, busca conductas observables alrededor de $theme, sobre todo lo que evitas, controlas en exceso o repites.',
      fr: 'Pour le travail de l’ombre, dans ${position.toLowerCase()}, cherchez les comportements observables autour de $theme, surtout ce que vous évitez, surcontrôlez ou répétez.',
      pt: 'No trabalho de sombra, em ${position.toLowerCase()}, procure comportamentos observáveis em torno de $theme, especialmente o que você evita, controla demais ou repete.',
    ),
    ReadingKind.compatibility => _copy(
      language,
      en: 'For compatibility, in ${position.toLowerCase()}, read $theme as a relational dynamic to observe between two people, not as a score.',
      tr: 'Uyum açılımında ${position.toLowerCase()} konumunda $theme temasını bir puan değil, iki kişi arasında gözlemlenecek ilişkisel bir dinamik olarak oku.',
      es: 'Para la compatibilidad, en ${position.toLowerCase()}, lee $theme como una dinámica relacional que observar entre dos personas, no como una puntuación.',
      fr: 'Pour la compatibilité, dans ${position.toLowerCase()}, lisez $theme comme une dynamique relationnelle à observer entre deux personnes, pas comme un score.',
      pt: 'Na compatibilidade, em ${position.toLowerCase()}, leia $theme como uma dinâmica relacional a observar entre duas pessoas, não como uma pontuação.',
    ),
    ReadingKind.timeline => _copy(
      language,
      en: 'On a timeline, in ${position.toLowerCase()}, $theme describes a condition that can change with choices; it does not name a fixed event.',
      tr: 'Zaman çizgisinde ${position.toLowerCase()} konumunda $theme, seçimlerle değişebilecek bir koşulu anlatır; sabit bir olayı ilan etmez.',
      es: 'En una línea de tiempo, en ${position.toLowerCase()}, $theme describe una condición que puede cambiar con las decisiones; no anuncia un hecho fijo.',
      fr: 'Sur une chronologie, dans ${position.toLowerCase()}, $theme décrit une condition qui peut changer avec les choix ; il n’annonce pas un événement figé.',
      pt: 'Em uma linha do tempo, em ${position.toLowerCase()}, $theme descreve uma condição que pode mudar com escolhas; não anuncia um evento fixo.',
    ),
    ReadingKind.celticCross => _copy(
      language,
      en: 'Within the wider Celtic Cross, in ${position.toLowerCase()}, read $theme in relation to the surrounding positions rather than in isolation.',
      tr: 'Geniş Kelt Haçı içinde ${position.toLowerCase()} konumunda $theme temasını tek başına değil, çevresindeki konumlarla ilişkili olarak oku.',
      es: 'Dentro de la Cruz Celta, en ${position.toLowerCase()}, lee $theme en relación con las posiciones que la rodean, no de forma aislada.',
      fr: 'Dans la Croix celtique, dans ${position.toLowerCase()}, lisez $theme en relation avec les positions voisines plutôt que de manière isolée.',
      pt: 'Na Cruz Celta, em ${position.toLowerCase()}, leia $theme em relação às posições ao redor, não de forma isolada.',
    ),
  };
  final orientation = reversed
      ? _copy(
          language,
          en: 'Because the card is reversed, in ${position.toLowerCase()}, look first for delay, excess, avoidance, or inward pressure around this theme.',
          tr: 'Kart ters olduğu için ${position.toLowerCase()} konumunda önce bu tema çevresinde gecikme, aşırılık, kaçınma veya içe dönük baskı olup olmadığına bak.',
          es: 'Como la carta está invertida, en ${position.toLowerCase()}, busca primero retraso, exceso, evitación o presión interna alrededor de este tema.',
          fr: 'Comme la carte est renversée, dans ${position.toLowerCase()}, cherchez d’abord retard, excès, évitement ou pression intérieure autour de ce thème.',
          pt: 'Como a carta está invertida, em ${position.toLowerCase()}, procure primeiro atraso, excesso, evitação ou pressão interna em torno desse tema.',
        )
      : _copy(
          language,
          en: 'Because the card is upright, in ${position.toLowerCase()}, start with the most usable and outwardly available expression of this theme.',
          tr: 'Kart düz olduğu için ${position.toLowerCase()} konumunda önce bu temanın kullanılabilir ve dışarıdan gözlemlenebilir ifadesine bak.',
          es: 'Como la carta está derecha, en ${position.toLowerCase()}, empieza por la expresión más utilizable y visible de este tema.',
          fr: 'Comme la carte est à l’endroit, dans ${position.toLowerCase()}, commencez par l’expression la plus utilisable et visible de ce thème.',
          pt: 'Como a carta está na posição normal, em ${position.toLowerCase()}, comece pela expressão mais utilizável e visível desse tema.',
        );
