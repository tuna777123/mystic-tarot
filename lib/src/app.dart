...
class _MysticAppState extends State<MysticApp> {
  ...
  Widget _shell() => Scaffold(
        body: IndexedStack(index: tab, children: [
          HomeScreen(
            ...
            streak: streak,
            xp: xp,
            dailyReadingDone: journal.any((record) =>
        ...
        builder: (_) => ReadingFlow(
            ...
            onComplete: (record) {
              ...
              setState(() {
                ...
                discoveredCards
                    .addAll(record.cards.map((item) => item.card.name));
                xp += 25;
  ...
  void _claimDailyQuest() {
    ...
    final readToday = journal.any((record) =>
        record.kind == ReadingKind.daily && _dayKey(record.createdAt) == today);
  ...
  void _completeArcanaChapter(int index, String reflection) {
    if (completedArcanaDays.contains(index) ||
        lastArcanaCompletionDay == _dayKey(DateTime.now())) return;
    setState(() {
  ...
  Future<void> _finishOnboarding(String name, String selectedIntention,
      MysticLanguage selectedLanguage) async {
    final cleanName = name.trim();
    setState(() {
      onboarded = true;
      userName = cleanName.length > 18 ? cleanName.substring(0, 18) : cleanName;
  ...
  Future<void> _loadProgress() async {
    try {
      ...
      setState(() {
        ...
        discoveredCards
            .addAll(prefs.getStringList('discovered_cards') ?? const []);
        ...
        if (ritualDay == today)
          completedRituals
              .addAll(prefs.getStringList('completed_rituals') ?? const []);
        ready = true;
      });
    } catch (_) {
  ...
  Future<void> _saveProgress() async {
    try {
      ...
      await Future.wait([
        ...
        prefs.setStringList(
            'claimed_rewards', claimedRewards.map((item) => '$item').toList()),
        prefs.setStringList('completed_arcana_days',
        ...
        prefs.setStringList(
            ...
            arcanaReflections.entries
                ...
                .toList()),
        prefs.setStringList(
            'journal_records', journal.take(50).map(_encodeRecord).toList()),
        prefs.setString('ritual_day', _dayKey(DateTime.now())),
        prefs.setString(
            'deep_readings_day', deepReadingsDay ?? _dayKey(DateTime.now())),
        prefs.setString('deck_style', deckStyle.name),
        prefs.setString('language', language.name),
        if (lastArcanaCompletionDay != null)
          prefs.setString(
              'last_arcana_completion_day', lastArcanaCompletionDay!),
  ...
  ReadingRecord? _decodeRecord(String encoded) {
    try {
      ...
      final cards = (data['cards'] as List<dynamic>).map((item) {
        ...
        final card =
            tarotDeck.firstWhere((candidate) => candidate.name == map['name']);
  ...
  Widget build(BuildContext context) => const Scaffold(
        ...
        SizedBox(
            ...
            child: CircularProgressIndicator(
                strokeWidth: 2, color: MysticColors.gold)),
      ]))));
}
        ...
        child: Material(
            ...
            child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                          color: color.withValues(alpha: .28), blurRadius: 55)
                    ]),
                child: AnimatedBuilder(
                    ...
                    builder: (context, _) =>
                        Column(mainAxisSize: MainAxisSize.min, children: [
                          Row(children: [
                            const Spacer(),
                            Text('${index + 1}/${widget.cards.length}',
                            ...
                            InkWell(
                                ...
                                child: const Icon(Icons.close,
                                    size: 20, color: MysticColors.muted))
                          ]),
                          const SizedBox(height: 5),
                          const Text('✦  NEW ARCANA AWAKENED',
                              style: TextStyle(
                                  fontFamily: 'Arial',
                          ...
                          Transform.scale(
                              scale: .72 +
                                  Curves.elasticOut.transform(controller.value) *
                                      .28,
                              child: Transform.rotate(
                                  angle: sin(controller.value * pi * 3) *
                                      (1 - controller.value) *
                                      .06,
                                  child: Container(
                                      width: 132,
                          ...
                          Text(_cardRarity(card).toUpperCase(),
                              style: TextStyle(
                                  fontFamily: 'Arial',
                                  color: color,
                                  fontSize: 9,
...
String _cardRarity(TarotCardData card) {
  ...
  if (card.name.startsWith('Page') ||
      ...
      card.name.startsWith('King')) return 'Epic';
      ...
      return Column(
          ...
          children: [
            ...
            Wrap(
              ...
              spacing: 8,
              runSpacing: 8,
              children: MysticLanguage.values
                  .map((item) => ChoiceChip(
        ...
        return SizedBox(
            ...
            child: Stack(alignment: Alignment.center, children: [
                  ...
                  child: const SizedBox(
                      ...
                      child: Stack(children: [
                        Align(
                            ...
                            child: Text('✦',
                                style: TextStyle(
                                    color: MysticColors.gold, fontSize: 13))),
                        Align(
                        ...
                        Align(
                            ...
                            child: Text('·',
                                style: TextStyle(
                                    color: MysticColors.gold, fontSize: 22))),
                        Align(
              ...
              Container(
                  ...
                  child: const Text('☾',
                      style:
                          TextStyle(fontSize: 42, color: MysticColors.gold))),
            ]));
      });
}
  ...
  Widget build(BuildContext context) => MysticBackground(
        ...
        SliverPadding(
            ...
            sliver: SliverList(
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Expanded(
                    child: Column(
                      ...
                      Text(
                          userName.isEmpty
                              ? mysticText(language, 'Your cards are waiting',
                ...
                InkWell(
                    ...
                    child: Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                color:
                                    MysticColors.gold.withValues(alpha: .4))),
              ...
              _ReadingAllowance(
                  readingsLeft: freeReadingsLeft, onUpgrade: onPremium),
                              ...
                              children: [
                                ...
                                Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 5),
                                      Text('${kind.cardCount} cards',
                                          style: Theme.of(context)
                                              .textTheme
                                              .bodyMedium)
                                    ])
                              ])));
                }, childCount: _standardReadingKinds.length),
                  ...
                  SizedBox(
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (_, index) => _premiumReadingCard(
                              context, _premiumReadingKinds[index]))),
                ]))),
      ]));
  ...
  Widget _premiumReadingCard(BuildContext context, ReadingKind kind) => InkWell(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              border:
                  Border.all(color: MysticColors.gold.withValues(alpha: .35))),
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Text(kind.symbol,
                  style:
                      const TextStyle(fontSize: 27, color: MysticColors.gold)),
              const Spacer(),
            ...
            const SizedBox(height: 5),
            Text('${kind.cardCount}-card premium spread',
                style: const TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.lavender,
                    fontSize: 10)),
...
class _PersonalSignal extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final record in recent) {
      emotionCounts.update(record.emotion, (value) => value + 1,
          ifAbsent: () => 1);
      for (final item in record.cards) {
        cardCounts.update(item.card.name, (value) => value + 1,
            ifAbsent: () => 1);
      }
    }
...
class _ReadingAllowance extends StatelessWidget {
  const _ReadingAllowance(
      {required this.readingsLeft, required this.onUpgrade});
      ...
      child: Container(
          ...
          child: Row(children: [
            Container(
                ...
                child: Icon(empty ? Icons.lock_outline : Icons.bolt,
            ...
            Text(empty ? 'VIEW PLUS' : '${3 - readingsLeft}/3',
                style: TextStyle(
                    fontFamily: 'Arial',
...
class _MoonBriefing extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          border:
              Border.all(color: MysticColors.lavender.withValues(alpha: .14)),
        ),
        child: Row(children: [
          Container(
              width: 42,
          ...
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                const Text('TONIGHT’S MYSTIC PULSE',
                    style: TextStyle(
                        fontFamily: 'Arial',
                        color: MysticColors.gold,
                        fontSize: 10,
                        ...
                        letterSpacing: 1.25)),
                const SizedBox(height: 4),
                Text('Release urgency. Choose the honest next step.',
                    style: Theme.of(context)
                        .textTheme
                        .bodyMedium
                        ?.copyWith(color: MysticColors.mist)),
...
class _DailyCard extends StatefulWidget {
  const _DailyCard(
      {required this.streak, required this.deckStyle, required this.onTap});
  final int streak;
  final DeckStyle deckStyle;
          ...
          child: Container(
              ...
              decoration: BoxDecoration(
                  gradient: LinearGradient(
                      ...
                      colors: [
                        Color.lerp(const Color(0xFF6C45B5),
                            const Color(0xFF8356C5), controller.value)!,
                        const Color(0xFF251944)
                      ]),
                  borderRadius: BorderRadius.circular(24),
                  border: Border.all(
                      color: MysticColors.lavender
                          .withValues(alpha: .32 + controller.value * .18)),
                  boxShadow: [
                    BoxShadow(
                        color: MysticColors.violet
                            .withValues(alpha: .12 + controller.value * .08),
                        blurRadius: 28,
                        spreadRadius: 1)
                  ]),
              child: Row(children: [
                Expanded(
                    child: Column(
                      ...
                      const SizedBox(height: 8),
                      Text('🔥 ${widget.streak} day streak  •  +25 XP',
                          style: Theme.of(context).textTheme.bodyMedium)
                    ])),
                TarotCardFace(style: widget.deckStyle, width: 90, height: 142)
              ]))));
}
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      child: Column(children: [
              ...
              child: Text(
                  claimed
                      ...
                      : complete
                          ...
                          : '☾',
                  style:
                      const TextStyle(color: MysticColors.gold, fontSize: 25))),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                const Text('DAILY SOUL QUEST',
                    style: TextStyle(
                        fontFamily: 'Arial',
                        color: MysticColors.gold,
                        fontSize: 10,
        ...
        Row(children: [
          _step(context, '1', 'Daily card', readingDone),
          const SizedBox(width: 8),
          _step(context, '2', 'One ritual', ritualDone)
        ]),
        const SizedBox(height: 12),
        ClipRRect(
            borderRadius: BorderRadius.circular(8),
              ...
              child: AnimatedBuilder(
                  ...
                  builder: (context, _) =>
                      Column(mainAxisSize: MainAxisSize.min, children: [
                              ...
                              Transform.scale(
                                  scale: .7 +
                                      Curves.elasticOut.transform(controller.value) *
                                          .3,
                                  child: Transform.rotate(
                                      angle: sin(controller.value * pi * 4) *
                                          (1 - controller.value) *
                                          .08,
                                      child: Container(
                                          width: 88,
                                          ...
                                          child: Text(
                                              controller.value > .55 ? '✦' : '◇',
                                              style: const TextStyle(fontSize: 43, color: MysticColors.ink))))),
                            ])),
                        const Text('SOUL CHEST OPENED',
                            style: TextStyle(
                                fontFamily: 'Arial',
                                color: MysticColors.gold,
                                fontSize: 11,
                        ...
                        const SizedBox(height: 8),
                        Text('Moon Shard added to your constellation.',
                            textAlign: TextAlign.center,
                            style: Theme.of(context).textTheme.bodyLarge),
                        const SizedBox(height: 20),
                        GoldButton(
...
class _ReadingFlowState extends State<ReadingFlow> {
  ...
  Widget _selection(BuildContext context) => Padding(
      ...
      child: Column(children: [
        Row(children: [
          ...
          const Spacer(),
          Text('${selected.length}/${widget.kind.cardCount}',
              style: const TextStyle(
                  fontFamily: 'Arial',
                  color: MysticColors.gold,
                  fontWeight: FontWeight.bold))
        ...
        TextField(
            ...
            decoration: const InputDecoration(
                hintText: 'Write your question (optional)',
        ...
        SizedBox(
            ...
            child: ListView.separated(
                scrollDirection: Axis.horizontal,
                itemCount: EmotionalState.values.length,
  ...
  void _prepareRitual() {
    ...
    setState(() {
      drawn = List.generate(widget.kind.cardCount,
    ...
    return CustomScrollView(slivers: [
      SliverAppBar(
          ...
          actions: [
            IconButton(
                onPressed:
                    revealComplete ? () => _openStoryStudio(record) : null,
                tooltip: mysticText(
                    widget.language, 'Share reading', 'Okumayı paylaş'),
      ...
      SliverPadding(
          ...
          sliver: SliverList(
            ...
            if (revealComplete)
              Container(
                  ...
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('MYSTIC MIRROR • 24H LOOP',
                            style: TextStyle(
                                fontFamily: 'Arial',
                                color: MysticColors.lavender,
                                fontWeight: FontWeight.bold,
            ...
            if (revealComplete)
              GoldButton(
                  label: saved ? 'Saved to your journal' : 'Save this reading',
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(
              ...
              style: const TextStyle(
                  ...
                  letterSpacing: 1.2)),
          const SizedBox(height: 6),
          Text('${card.card.name}${card.reversed ? ' — Reversed' : ''}',
              style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 8),
  ...
  String _headline() {
    final hopeful = drawn!
        .any((c) => c.card.name == 'The Star' || c.card.name == 'The Sun');
  ...
  Widget _memoryBridge(BuildContext context) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(20),
            border: Border.all(
                color: MysticColors.lavender.withValues(alpha: .26))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('◉  ORACLE MEMORY',
              style: TextStyle(
                  fontFamily: 'Arial',
                  color: MysticColors.lavender,
                  fontSize: 10,
  ...
  Widget _oracleInvitation(BuildContext context, ReadingRecord record) =>
      InkWell(
        onTap: oracleQuestionUsed
            ? widget.onPremium
            : () => Navigator.push(
                context,
                MaterialPageRoute(
        ...
        child: Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                    color: MysticColors.gold.withValues(alpha: .36))),
            child: Row(children: [
              Container(
                  width: 48,
                  height: 48,
                  ...
                  child: Text(oracleQuestionUsed ? '✦' : '◉',
                      style: const TextStyle(
                          fontSize: 25, color: MysticColors.gold))),
              const SizedBox(width: 13),
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    Row(children: [
                      const Text('ASK THE ORACLE',
                          style: TextStyle(
                              ...
                              letterSpacing: 1)),
                      const Spacer(),
                      Text(oracleQuestionUsed ? 'CONTINUE PLUS' : '1 FREE',
                          style: const TextStyle(
                              fontFamily: 'Arial',
                              color: MysticColors.lavender,
                              fontSize: 8,
...
class _RevealRitualState extends State<_RevealRitual>
  ...
  @override
  Widget build(BuildContext context) =>
      ListView(padding: const EdgeInsets.fromLTRB(20, 10, 20, 28), children: [
        Row(children: [
          IconButton(
              onPressed: opening ? null : widget.onBack,
              icon: const Icon(Icons.arrow_back)),
          const Spacer(),
          const Text('REVEAL RITUAL',
              style: TextStyle(
                  fontFamily: 'Arial',
                  color: MysticColors.gold,
                  fontWeight: FontWeight.w900,
        ...
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _ritualStep('01', 'INTENTION', true)),
          const SizedBox(width: 7),
          Expanded(child: _ritualStep('02', 'SELECTION', true)),
          const SizedBox(width: 7),
          Expanded(child: _ritualStep('03', 'REVEAL', opening))
        ]),
        const SizedBox(height: 28),
                      ...
                      child: Stack(alignment: Alignment.center, children: [
                        Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                    color: MysticColors.gold
                                        .withValues(alpha: .08 + value * .14)),
                                boxShadow: [
                                  BoxShadow(
                                      color: MysticColors.violet
                                          .withValues(alpha: .13 + value * .09),
                                      blurRadius: 46,
                                      spreadRadius: 8)
                                ])),
                          ...
                          child: AnimatedOpacity(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                    color: const Color(0xFFFFE5A0), width: 2),
                                boxShadow: [
                                  BoxShadow(
                                      color: MysticColors.gold
                                          .withValues(alpha: .32 + value * .18),
                                      blurRadius: 30)
                                ],
                              ),
                              child: const Text('✦',
                                  style: TextStyle(
                                      color: MysticColors.ink, fontSize: 27)),
                            ),
                          ),
                        ),
                      ])));
        ...
        Center(
            child: Container(
                ...
                child: Text(
                    '${widget.emotion.symbol}  ${widget.emotion.label.toUpperCase()}  •  ${widget.cardCount} ${widget.cardCount == 1 ? 'CARD' : 'CARDS'}',
                    style: const TextStyle(
                        fontFamily: 'Arial',
                        color: MysticColors.lavender,
                        fontSize: 9,
        ...
        const SizedBox(height: 26),
        GoldButton(
            label: opening ? 'Opening the veil…' : 'Open the seal',
            icon: opening
                ? Icons.hourglass_top_rounded
                : Icons.touch_app_outlined,
            onPressed: opening ? null : _open),
        const SizedBox(height: 10),
        Text('Take what resonates. The cards offer reflection, not certainty.',
            textAlign: TextAlign.center,
            style:
...
class _OracleDialogueScreenState extends State<OracleDialogueScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
            title: Text(mysticText(
                widget.language, 'Oracle Dialogue', 'Oracle Diyaloğu'))),
        body: MysticBackground(
            child: ListView(
              ...
              Center(
                  child: Container(
                      ...
                      child: const Text('◉',
                          style: TextStyle(
                              fontSize: 35, color: MysticColors.ink)))),
              const SizedBox(height: 13),
              Text(
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.memory,
                        size: 17, color: MysticColors.lavender),
                        ...
                        child: Container(
                            ...
                            child: Row(children: [
                              ...
                              const Icon(Icons.arrow_forward_ios,
                                  size: 13, color: MysticColors.gold)
                            ]))))),
                const SizedBox(height: 5),
                TextField(
                    ...
                    decoration: const InputDecoration(
                        hintText: 'Or ask in your own words…',
                        prefixIcon: Icon(Icons.chat_bubble_outline))),
                const SizedBox(height: 9),
                GoldButton(
                    label: 'Ask my free question',
                    icon: Icons.auto_awesome,
              ...
              if (askedQuestion != null) ...[
                      ...
                      child: const Row(children: [
                        SizedBox(
                            ...
                            child: CircularProgressIndicator(
                                strokeWidth: 2, color: MysticColors.gold)),
                        SizedBox(width: 12),
                        Text('The Oracle is connecting your symbols…',
                            style: TextStyle(
                                fontFamily: 'Arial',
                                color: MysticColors.lavender,
                                fontSize: 11))
                      ...
                      child: Column(children: [
                        const Row(children: [
                          Icon(Icons.lock_outline,
                              size: 19, color: MysticColors.gold),
                          SizedBox(width: 9),
                          Expanded(
                              child: Text('Continue the conversation',
                                  style: TextStyle(
                                      fontSize: 16,
                                      fontWeight: FontWeight.w700)))
                        ]),
                ...
                if (answer != null)
                  TextButton(
                      onPressed: () => Navigator.pop(context),
                      child: const Text('Return to my reading')),
              ],
            ])));
  }

  Widget _messageBubble(BuildContext context, String text,
...
class _RitualRevealCard extends StatefulWidget {
  const _RitualRevealCard(
      {required this.card, required this.deckStyle, required this.delay});
...
class _JourneyScreenState extends State<JourneyScreen>
        ...
        child: ListView(
          ...
          Row(children: [
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text('Your Mystic Path',
                      style: Theme.of(context).textTheme.headlineMedium),
                  const SizedBox(height: 5),
                  Text('Your inner world becomes visible as you practice.',
                      style: Theme.of(context).textTheme.bodyMedium)
                ])),
            Container(
                padding:
          ...
          AnimatedBuilder(
              ...
              builder: (context, _) => Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: MysticColors.lavender
                                .withValues(alpha: .18 + glow.value * .18)),
                        boxShadow: [
                          BoxShadow(
                              color: MysticColors.violet
                                  .withValues(alpha: .12 + glow.value * .08),
                              blurRadius: 32)
                        ]),
                    child: Stack(children: [
                      Positioned.fill(
                      ...
                      Positioned(
                          ...
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('INNER CONSTELLATION',
                                    style: TextStyle(
                                        fontFamily: 'Arial',
                                        color: MysticColors.gold,
                                        fontWeight: FontWeight.w800,
          ...
          Row(children: [
            Text('Today’s rituals',
                style: Theme.of(context).textTheme.titleLarge),
            const Spacer(),
            Text('${widget.completedRituals.length}/3',
                style: const TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.gold,
                    fontWeight: FontWeight.bold))
          ...
          Text(
              ...
              style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          _ritual(context, 'breathe', '60-second reset',
              'Breathe in for four, out for six.', Icons.air),
          _ritual(context, 'truth', 'Name the truth',
              'Write one sentence you have been avoiding.', Icons.edit_note),
          _ritual(context, 'action', 'Aligned action',
              'Take the smallest reversible next step.', Icons.bolt),
          const SizedBox(height: 15),
          Text('Mystic rewards', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 6),
          Text('Your practice unlocks cosmetic relics—never better answers.',
              style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          SizedBox(
              height: 145,
              ...
              child: Row(children: [
                ...
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      Text('${widget.streak}-day flame',
                          style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 4),
                      Text('Return tomorrow to keep your constellation alive.',
                          style: Theme.of(context).textTheme.bodyMedium)
                    ]))
              ])),
        ]));
  ...
  Widget _reward(
      BuildContext context, int milestone, String title, String symbol) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(18),
            border: Border.all(
                color: claimed ? MysticColors.gold : Colors.white12)),
        child: Column(children: [
          Text(
              claimed
                  ? symbol
  ...
  Widget _ritual(BuildContext context, String id, String title, String subtitle,
      IconData icon) {
                    ...
                    child: Column(
                      ...
                      Text(title,
                          style: const TextStyle(
                              ...
                              fontWeight: FontWeight.w700)),
                      const SizedBox(height: 3),
                      Text(done ? 'Ritual complete • +15 XP' : subtitle,
                          style: Theme.of(context).textTheme.bodyMedium)
                    ])),
                if (!done)
                  const Icon(Icons.arrow_forward_ios,
                      size: 14, color: MysticColors.muted),
              ])),
        ));
  }
}
...
class _ArcanaVault extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final unlocked =
        tarotDeck.where((card) => discoveredCards.contains(card.name)).toList();
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(22),
            border:
                Border.all(color: MysticColors.gold.withValues(alpha: .24))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Text('ARCANA VAULT',
                style: TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.gold,
                    fontSize: 11,
                    ...
                    letterSpacing: 1.35)),
            const Spacer(),
            Text('${discoveredCards.length}/78',
                style: const TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.lavender,
                    fontSize: 12,
            ...
            const SizedBox(width: 5),
            const Icon(Icons.arrow_forward_ios,
                size: 12, color: MysticColors.muted),
          ]),
          const SizedBox(height: 6),
          Text(
          ...
          Row(children: [
            for (var i = 0; i < preview.length; i++) ...[
              ...
              Expanded(
                  child: _previewCard(
                      preview[i], discoveredCards.contains(preview[i].name)))
            ]
          ]),
        ]),
      ),
  ...
  Widget _previewCard(TarotCardData card, bool unlocked) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(
                color:
                    unlocked ? color.withValues(alpha: .55) : Colors.white10)),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text(unlocked ? card.symbol : '◈',
              style: TextStyle(
                  fontSize: 28, color: unlocked ? color : Colors.white24)),
          const SizedBox(height: 8),
          Text(unlocked ? card.name : 'Undiscovered',
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              textAlign: TextAlign.center,
              style: TextStyle(
      ...
      child: Container(
          ...
          child: Column(children: [
            Align(
                ...
                child: Text(unlocked ? card.number : '—',
                    style: TextStyle(
                        fontFamily: 'Arial',
            ...
            Text(unlocked ? card.symbol : '?',
                style: TextStyle(
                    fontSize: 30, color: unlocked ? color : Colors.white24)),
            const Spacer(),
            Text(unlocked ? card.name : 'Locked',
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
                textAlign: TextAlign.center,
                style: TextStyle(
                    ...
                    fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            Text(unlocked ? _cardRarity(card).toUpperCase() : 'UNDISCOVERED',
                style: TextStyle(
                    fontFamily: 'Arial',
                    color: unlocked ? color : Colors.white24,
                    fontSize: 7,
                ...
                child: SingleChildScrollView(
                  ...
                  Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(18),
                          border:
                              Border.all(color: color.withValues(alpha: .75)),
                          boxShadow: [
                            BoxShadow(
                  ...
                  const SizedBox(height: 20),
                  _meaningBlock(
                      dialogContext, 'LIGHT', card.light, MysticColors.gold),
                  const SizedBox(height: 10),
                  _meaningBlock(dialogContext, 'SHADOW', card.shadow,
                      MysticColors.lavender),
                  const SizedBox(height: 10),
                  _meaningBlock(
                      dialogContext, 'ALIGNED ACTION', card.advice, color),
                ])))));
  }
...
class _WeeklyMirror extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final recent =
        records.where((record) => record.createdAt.isAfter(weekAgo)).toList();
          ...
          child: Row(children: [
            Container(
                ...
                decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle),
                child: Text(recent.isEmpty ? '☾' : emotion.symbol,
                    style: const TextStyle(
                        fontSize: 26, color: MysticColors.gold))),
            const SizedBox(width: 14),
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  const Text('YOUR WEEKLY MYSTIC WRAPPED',
                      style: TextStyle(
                          fontFamily: 'Arial',
                          color: MysticColors.gold,
                          fontSize: 10,
  ...
  void _showWrapped(BuildContext context, List<ReadingRecord> recent,
      EmotionalState emotion, String card) {
    showModalBottomSheet<void>(
        context: context,
        isScrollControlled: true,
        ...
        builder: (context) => Padding(
            ...
            child: SafeArea(
              ...
              const SizedBox(height: 22),
              Row(children: [
                _wrappedStat(context, '${recent.length}', 'REFLECTIONS'),
                const SizedBox(width: 10),
                _wrappedStat(context, card, 'REPEATING CARD')
              ]),
              const SizedBox(height: 16),
              Text(
                  recent.isEmpty
...
class _ConstellationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < points.length; i++) {
      ...
      if (on) {
        canvas.drawCircle(point, 7 + pulse * 3,
      ...
      canvas.drawCircle(
          ...
          Paint()
            ..color =
                on ? MysticColors.gold : Colors.white.withValues(alpha: .15));
    }
  }
      ...
      child: Padding(
          ...
          child:
            ...
            Text('Your journal',
                style: Theme.of(context).textTheme.headlineMedium),
            const SizedBox(height: 6),
            Text('A record of the patterns you are learning to see.',
                style: Theme.of(context).textTheme.bodyMedium),
            const SizedBox(height: 22),
            Expanded(
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Container(
                              ...
                              child: Row(children: [
                                CircleAvatar(
                                    backgroundColor: MysticColors.violet
                                        .withValues(alpha: .35),
                                    child: Text(item.emotion.symbol,
                                        style: const TextStyle(
                                            color: MysticColors.gold))),
                                const SizedBox(width: 14),
                                Expanded(
...
class _EmptyJournal extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        ...
        Text('Your story begins here',
            style: Theme.of(context).textTheme.titleLarge),
        const SizedBox(height: 7),
        Text('Save a reading and it will appear in your private journal.',
            textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.bodyMedium)
      ]));
}
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MysticBackground(
        child: ListView(
          ...
          Container(
              ...
              child: Column(children: [
                Container(
                    ...
                    child: const Text('☾',
                        style:
                            TextStyle(fontSize: 34, color: MysticColors.gold))),
                const SizedBox(height: 12),
                Text(userName.isEmpty ? _titleForLevel(level) : userName,
                    style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(height: 4),
                Text(
                    '${_titleForLevel(level).toUpperCase()} • $intention PATH • LEVEL $level',
                    textAlign: TextAlign.center,
                    style: const TextStyle(
                        fontFamily: 'Arial',
                        color: MysticColors.gold,
                ...
                Row(
                    ...
                    children: [
                      ...
                      _stat('$readings', 'readings'),
                      _stat('$discovered', 'arcana')
                    ]),
                const SizedBox(height: 18),
                Row(children: [
                  Text('$xp XP',
                      style: const TextStyle(
                          ...
                          fontWeight: FontWeight.bold)),
                  const Spacer(),
                  Text('${100 - (xp % 100)} XP to level ${level + 1}',
                      style: const TextStyle(
                          fontFamily: 'Arial',
                          color: MysticColors.muted,
                          fontSize: 10))
          ...
          const SizedBox(height: 22),
          Row(children: [
            Text('Mystic achievements',
                style: Theme.of(context).textTheme.titleLarge),
            const Spacer(),
            Text('${badges.where((badge) => badge.$3).length}/${badges.length}',
                style: const TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.gold,
                    fontWeight: FontWeight.bold))
          ...
          const SizedBox(height: 6),
          Text('Your practice leaves permanent marks on your path.',
              style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          GridView.count(
              shrinkWrap: true,
              ...
              children: badges
                  ...
                  .toList()),
          const SizedBox(height: 24),
          Text('Your tarot deck',
              style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 6),
          Text('Choose the visual energy that follows every reading.',
              style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          SizedBox(
              height: 178,
          ...
          InkWell(
              ...
              child: Container(
                  ...
                  child: const Row(children: [
                    Text('✦',
                        style:
                            TextStyle(fontSize: 28, color: MysticColors.gold)),
                    SizedBox(width: 14),
                    Expanded(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                          Text('Unlock Mystic Plus',
                              style: TextStyle(
                                  fontSize: 18, fontWeight: FontWeight.bold)),
                          SizedBox(height: 4),
                          Text('Go deeper with unlimited readings',
                              style: TextStyle(
                                  fontFamily: 'Arial',
                                  color: MysticColors.lavender))
                        ])),
          ...
          ListTile(
              ...
              title: Text(mysticText(language, 'Language', 'Dil'),
          ...
          ...[
            ...
            'Help and support'
              ...
              leading: const Icon(Icons.auto_awesome_outlined,
                  color: MysticColors.lavender),
              title: Text(label, style: const TextStyle(fontFamily: 'Arial')),
              trailing: const Icon(Icons.chevron_right))),
        ]));
  }

  Widget _mysticSoulCard(
      BuildContext context, MysticIdentitySnapshot identity) {
    final appLanguage = language.appLanguage;
    final title = localized(appLanguage,
    ...
    final action = localized(appLanguage,
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                const SizedBox(height: 9),
                Row(children: [
                  Text('$action • ${identity.confidence}%',
                      style: const TextStyle(
                          fontFamily: 'Arial',
                          color: MysticColors.lavender,
                          fontSize: 9,
                  ...
                  const Icon(Icons.arrow_forward,
                      size: 14, color: MysticColors.gold)
                ]),
              ])),
        ]),
      ),
              ...
              Center(
                  child: Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(8)))),
              const SizedBox(height: 20),
              Text(mysticText(language, 'Choose language', 'Dil seç'),
                  style: Theme.of(sheetContext).textTheme.headlineMedium),
              const SizedBox(height: 8),
              Text(
                  mysticText(
                      language,
                      'Your flagship journey and navigation update instantly. Full reading translations continue expanding with each release.',
                      'Ana yolculuğun ve menüler anında güncellenir. Okuma çevirileri her sürümde genişlemeye devam eder.'),
                  style: Theme.of(sheetContext).textTheme.bodyMedium),
              const SizedBox(height: 14),
              ...MysticLanguage.values.map((item) => ListTile(
                    onTap: () {
  ...
  Widget _badge(BuildContext context, String title, String symbol,
          bool unlocked, String goal) =>
      AnimatedContainer(
          duration: const Duration(milliseconds: 350),
          padding: const EdgeInsets.all(13),
          decoration: BoxDecoration(
              gradient: unlocked
                  ? const LinearGradient(
                      colors: [Color(0xFF4A326D), Color(0xFF21172F)])
                  : const LinearGradient(
                ...
                child: Column(
                  ...
                  Text(title,
                      ...
                      style: TextStyle(
                          fontFamily: 'Arial',
                          color:
                              unlocked ? MysticColors.mist : MysticColors.muted,
                          fontSize: 11,
                          fontWeight: FontWeight.w800)),
                  const SizedBox(height: 4),
                  Text(unlocked ? 'UNLOCKED' : goal,
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
                          fontFamily: 'Arial',
                          color:
                              unlocked ? MysticColors.gold : MysticColors.muted,
                          fontSize: 8,
                          fontWeight: FontWeight.bold,
  ...
  Widget _deckOption(BuildContext context, DeckStyle style) {
    final unlocked = style == DeckStyle.midnight ||
          ...
          child: Column(children: [
            Align(
                ...
                child: Icon(
                    ...
                    size: 16)),
            Text(unlocked ? style.symbol : '◈',
                style: TextStyle(
                    fontSize: 34, color: unlocked ? accent : Colors.white24)),
            const Spacer(),
            Text(style.label,
                textAlign: TextAlign.center,
                style: TextStyle(
                    ...
                    fontWeight: FontWeight.w800)),
            const SizedBox(height: 5),
            Text(active ? 'ACTIVE DECK' : style.subtitle,
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
                textAlign: TextAlign.center,
                style: TextStyle(
  ...
  Widget _stat(String value, String label) => Column(children: [
        ...
        Text(label,
            style: const TextStyle(
                fontFamily: 'Arial', color: MysticColors.muted, fontSize: 12))
      ]);
}
...
class _SoulProfileScreenState extends State<SoulProfileScreen> {
  ...
  Widget build(BuildContext context) {
    const choices = ['Clarity', 'Love', 'Purpose', 'Healing'];
    return Scaffold(
        appBar: AppBar(title: const Text('Soul profile')),
        body: MysticBackground(
            child: ListView(
              ...
              const Center(
                  child: Text('◉',
                      style:
                          TextStyle(fontSize: 58, color: MysticColors.gold))),
              const SizedBox(height: 12),
              Text('Make Mystic yours',
                  textAlign: TextAlign.center,
                  style: Theme.of(context).textTheme.headlineMedium),
              const SizedBox(height: 8),
              Text(
              ...
              const SizedBox(height: 10),
              const Text('Stored privately on this device.',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      fontFamily: 'Arial',
                      color: MysticColors.muted,
...
class _MysticSettingsScreenState extends State<MysticSettingsScreen> {
  ...
  List<Widget> _content(BuildContext context) {
    if (widget.section == 'Reading preferences') {
      return [
        _intro(context, 'Shape every reading',
            'Mystic should adapt to your practice—not ask you to adapt to it.'),
        SwitchListTile(
            contentPadding: EdgeInsets.zero,
            title: const Text('Allow reversed cards'),
            subtitle: const Text(
        ...
        SwitchListTile(
            contentPadding: EdgeInsets.zero,
            title: const Text('Ritual sound effects'),
            subtitle: const Text(
        ...
        const ListTile(
            ...
            leading: Icon(Icons.psychology_alt_outlined,
                color: MysticColors.lavender),
            title: Text('Reflection-first guidance'),
            subtitle: Text(
    ...
    if (widget.section == 'Daily reminder') {
      return [
        _intro(context, 'Protect your daily ritual',
            'A gentle return works better than endless notifications.'),
        SwitchListTile(
            contentPadding: EdgeInsets.zero,
            title: const Text('Daily Guidance reminder'),
            subtitle: Text(reminderEnabled
                ? 'Scheduled for 9:00 AM on supported mobile devices.'
                : 'Off'),
        ...
        const SizedBox(height: 12),
        _notice('Web preview note',
            'Notification permission and system scheduling activate in the native iOS/Android release.'),
      ];
    }
    if (widget.section == 'Privacy & data') {
      return [
        _intro(context, 'Your inner world stays yours',
            'This preview keeps your journal and progress locally on this device. No questions are sold to advertisers.'),
        ListTile(
            contentPadding: EdgeInsets.zero,
            leading:
                const Icon(Icons.download_outlined, color: MysticColors.gold),
            title: const Text('Export my journal'),
            subtitle: Text('${widget.records.length} saved readings'),
            trailing: const Icon(Icons.chevron_right),
        ...
        ListTile(
            ...
            title: const Text('Delete all Mystic data'),
            subtitle: const Text(
                'Permanently removes local journal, XP, streak, and settings.'),
            onTap: _confirmDelete),
        const SizedBox(height: 14),
        _notice('Entertainment & reflection',
    ...
    return [
      ...
      _faq('How do I restore a purchase?',
  ...
  Widget _notice(String title, String body) => Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(body,
            style: const TextStyle(
                fontFamily: 'Arial', color: MysticColors.muted, height: 1.45))
      ]));
  ...
  Future<void> _exportJournal() async {
    final text = widget.records.isEmpty
        ...
        : widget.records
            .map((record) =>
                '${record.createdAt.toLocal()} — ${record.kind.title}\n${record.cards.map((item) => '${item.card.name}${item.reversed ? ' (Reversed)' : ''}').join(', ')}\nAligned action: ${record.alignedAction}')
            .join('\n\n');
    await Clipboard.setData(ClipboardData(text: text));
    if (mounted)
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Your journal was copied for export.')));
  }
                    ...
                    actions: [
                      ...
                      TextButton(
                          ...
                          child: const Text('Delete everything',
                              style: TextStyle(color: Color(0xFFFF8090))))
                    ])) ??
        false;
...
class _PremiumReadingPreviewState extends State<PremiumReadingPreview> {
  ...
  Widget build(BuildContext context) => Scaffold(
          body: MysticBackground(
              ...
              firstChild: const Center(
                ...
                SizedBox(
                    ...
                    child: CircularProgressIndicator(
                        strokeWidth: 2, color: MysticColors.gold)),
                SizedBox(width: 10),
                Text('The first signal is forming…',
                    style: TextStyle(
                        fontFamily: 'Arial',
                        color: MysticColors.lavender,
                        fontSize: 11))
                  ...
                  child: Column(children: [
                    const Text('YOUR FIRST SIGNAL',
                        style: TextStyle(
                            ...
                            letterSpacing: 1.25)),
                    const SizedBox(height: 8),
                    Text(previewCard.card.name,
                        style: const TextStyle(
                            fontSize: 18, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 7),
                    Text(
                        previewCard.reversed
                            ? previewCard.card.shadow
                            : previewCard.card.light,
                        textAlign: TextAlign.center,
                        style: Theme.of(context).textTheme.bodyLarge),
                    const SizedBox(height: 8),
                    Text(
            ...
            const SizedBox(height: 22),
            Row(children: [
              Text('The rest of your spread',
                  style: Theme.of(context).textTheme.titleLarge),
              const Spacer(),
              Text('${widget.kind.cardCount - 1} LOCKED',
                  style: const TextStyle(
                      fontFamily: 'Arial',
                      color: MysticColors.gold,
                      fontSize: 8,
            ...
            const SizedBox(height: 22),
            GoldButton(
                label: 'Unlock the full ${widget.kind.title}',
                onPressed: widget.onUnlock,
                icon: Icons.auto_awesome),
            const SizedBox(height: 9),
            const Text('Included with Mystic Plus • Cancel anytime',
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.muted,
...
class _PremiumScreenState extends State<PremiumScreen> {
          ...
          body: MysticBackground(
            ...
            Container(
                ...
                decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    gradient: const RadialGradient(
                        colors: [Color(0xFFFFE5A3), Color(0xFF9D6D26)]),
                    boxShadow: [
                      BoxShadow(
                          color: MysticColors.gold.withValues(alpha: .32),
                          blurRadius: 32)
                    ]),
                child: const Text('✦',
                    style: TextStyle(fontSize: 39, color: MysticColors.ink))),
            const SizedBox(height: 8),
            if (widget.source == 'premium_spread')
              const Text('PREMIUM SPREAD COLLECTION',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      fontFamily: 'Arial',
                      color: MysticColors.gold,
            ...
            if (widget.source == 'oracle_dialogue')
              const Text('UNLIMITED ORACLE DIALOGUE',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      fontFamily: 'Arial',
                      color: MysticColors.gold,
            ...
            Text(
                widget.source == 'daily_limit'
                    ? 'Your insight does not\nhave to stop here.'
                    : widget.source == 'premium_spread'
                        ? 'Some questions need\na deeper spread.'
                        : widget.source == 'oracle_dialogue'
                            ? 'The first answer opened\na deeper conversation.'
                            : 'Make space for\ndeeper insight.',
                textAlign: TextAlign.center,
                style: Theme.of(context).textTheme.displaySmall),
            const SizedBox(height: 12),
            Text(
                widget.source == 'daily_limit'
                    ? 'You used today’s three free deep readings. Daily Guidance stays free—or unlock every spread without limits.'
                    : widget.source == 'premium_spread'
                        ? 'Unlock Love Compatibility, Future Timeline, Celtic Cross, and every premium reading in one membership.'
                        : widget.source == 'oracle_dialogue'
                            ? 'Continue asking follow-ups, keep your conversations, and let every answer build on the cards and patterns already in your private practice.'
                            : 'Turn occasional readings into a private practice that grows more useful every day.',
                textAlign: TextAlign.center,
                style: Theme.of(context).textTheme.bodyLarge),
            const SizedBox(height: 24),
            if (kIsWeb)
              Container(
                  ...
                  child: const Row(children: [
                    Icon(Icons.language, size: 18, color: MysticColors.gold),
                    SizedBox(width: 10),
                    Expanded(
                        child: Text(
                            'WEB EARLY ACCESS • No payment is collected in this release.',
                            style: TextStyle(
                                fontFamily: 'Arial',
                                color: MysticColors.lavender,
                                fontSize: 10,
            ...
            ...[
              'Unlimited readings and every spread',
              'Unlimited Oracle follow-up dialogue',
              'Complete journal and weekly pattern history',
              'All premium tarot deck themes',
              'No ads—ever'
            ].map((item) => Padding(
                ...
                child: Row(children: [
                  const CircleAvatar(
                      ...
                      backgroundColor: MysticColors.gold,
                      child:
                          Icon(Icons.check, size: 14, color: MysticColors.ink)),
                  const SizedBox(width: 11),
                  Expanded(
                      child: Text(item,
                          style: Theme.of(context).textTheme.bodyLarge))
            ...
            SizedBox(
                ...
                child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(
                          child: _plan(1, 'Monthly', r'$9.99', 'per month')),
                      const SizedBox(width: 8),
                      Expanded(
                          child: _plan(2, 'Yearly', r'$39.99', r'$3.33/month',
                              badge: 'SAVE 67%')),
                    ])),
            const SizedBox(height: 18),
            GoldButton(
                label: kIsWeb
                    ? 'Mystic Plus mobile launch'
                    : plan == 2
                        ? 'Start my 7-day free trial'
                        : 'Continue with ${plan == 0 ? 'weekly' : 'monthly'}',
                onPressed: _beginPurchase,
                icon: kIsWeb
            ...
            Text(
                kIsWeb
                    ? 'Planned pricing is shown for transparency. Final localized pricing and trial eligibility will appear in the official store checkout.'
                    : plan == 2
                        ? r'No charge today. Then $39.99/year unless cancelled.'
            ...
            const Text(
                kIsWeb
                    ? 'This public release cannot charge your card or activate a subscription.'
                    : 'Subscription purchases use the official Apple or Google checkout.',
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontFamily: 'Arial',
                    color: MysticColors.muted,
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              TextButton(
                  onPressed: () => _openLegal('Terms of Use'),
                  child: const Text('Terms')),
              const Text('•', style: TextStyle(color: MysticColors.muted)),
              TextButton(
                  onPressed: () => _openLegal('Privacy Policy'),
                  child: const Text('Privacy'))
            ]),
          ])));

  Widget _plan(int index, String title, String price, String subtitle,
      {String? badge}) {
    ...
    return InkWell(
        ...
        child: Stack(clipBehavior: Clip.none, children: [
          Container(
              padding: const EdgeInsets.fromLTRB(8, 17, 8, 12),
              decoration: BoxDecoration(
                  color: active
                      ? MysticColors.violet.withValues(alpha: .35)
                      : Colors.white.withValues(alpha: .04),
        ...
        builder: (context) => SafeArea(
            child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(
                      kIsWeb
                          ? 'Mystic Plus is preparing for mobile.'
                          : 'Store checkout is being connected.',
                      textAlign: TextAlign.center,
                      style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 9),
                  Text(
                      kIsWeb
                          ? 'The web release is free early access and cannot charge you. Paid access will use the official Apple or Google checkout.'
                          : 'Purchases remain unavailable until the store products and merchant accounts are connected.',
                      textAlign: TextAlign.center,
                      style: Theme.of(context).textTheme.bodyMedium),
                  const SizedBox(height: 18),
                  GoldButton(
  ...
  void _restore() {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text(
            'Restore Purchases activates with the native store billing connection.')));
  }

  void _openLegal(String title) => Navigator.push(context,
      MaterialPageRoute(builder: (_) => LegalDocumentScreen(title: title)));
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: MysticBackground(
          ...
          Text(privacy ? 'Privacy, in plain language' : 'A fair mystical space',
              style: Theme.of(context).textTheme.headlineMedium),
          const SizedBox(height: 12),
          Text(
              privacy
                  ? 'Mystic Tarot stores onboarding choices, journal entries, progress, and preferences locally on your device. The current release does not create an account, transmit journal content to us, sell personal data, include advertising trackers, or process payments. You can export or delete your local data from Privacy & data.'
                  : 'Mystic Tarot is a self-reflection and entertainment product. Readings are symbolic prompts, not factual predictions or substitutes for medical, mental-health, legal, financial, or emergency advice. The current web release does not process payments.',
              style: Theme.of(context).textTheme.bodyLarge),
          const SizedBox(height: 20),
          _legalSection(
              context,
              privacy ? 'Payments and future services' : 'Subscriptions',
              privacy
                  ? 'The public web release does not process payments. Before accounts, analytics, AI services, cloud sync, or native subscriptions are activated, this policy and the store disclosures will identify each provider, purpose, retention period, and deletion method.'
                  : 'The public web release does not process payments. If native subscriptions launch, the exact localized price, renewal period, trial eligibility, billing date, cancellation controls, and refund rules shown by Apple or Google at confirmation will control the purchase.'),
          _legalSection(
              context,
              privacy ? 'Your control' : 'Acceptable use',
              privacy
                  ? 'You may export your journal and permanently delete all local Mystic data at any time from the profile. Deleting browser storage or uninstalling the preview may also remove local records.'
                  : 'Do not rely on a reading for emergencies or high-stakes decisions. You remain responsible for your choices and should consult a qualified professional where appropriate.'),
          const SizedBox(height: 12),
          const Text('Effective July 23, 2026',
              style: TextStyle(
                  fontFamily: 'Arial',
                  color: MysticColors.muted,
                  fontSize: 10)),
